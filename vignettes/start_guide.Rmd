...
Once installed, load the package as follows. Note, packages vegan, maps and mapdata are required for **ausplotsR** to load, and functions are also imported from packages: plyr, R.utils, simba, httr, jsonlite, sp, maptools, ggplot2, gtools, jose, curl and betapart, while knitr, rmarkdown and goeveg are required to build this package vignette (i.e., if 'build_vignettes' is set to TRUE above). 
