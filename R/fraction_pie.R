fraction_pie <- function(frac, row) {
	try(pie(x=as.numeric(frac[row,c(2:ncol(frac))]), labels=as.numeric(frac[row,c("bare", "brown", "green", "other")[1:(ncol(frac)-1)]]), main=frac$site_unique[row], col=c("brown", "orange", "darkgreen", "gray")))
