...
total.points.fun <- function(x) {return(length(unique(hits[which(hits$site_unique == x),]$hits_unique)))} #function to go through a list of plot names and count how many unique hits there were
...
covers <- count(hits[which(as.character(hits$in_canopy_sky) == "FALSE"),], c("site_unique", "herbarium_determination")) #counts number of rows with same site and species name, last colname 'freq', i.e. counting the number of hits for a given species in a given plot. In canopy sky hits are removed (==TRUE) first so that projected foliage cover rather than opaque canopy cover is calculated. IF YOU WANT opaque canopy cover, would need remove that part - would be neater to contian within a function where this is an argument, such as cover.type=c("PFC", "opaque")
