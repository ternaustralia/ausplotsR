...
hits$standard <- hits$transect #copy the vector which says which PI transect the hit was on so we can have the original one as well as the altered one [IMPORTANT because for some situations, whether the transect was sampled in one direction or the other may be relevat, e.g. reconstructing the plot, whereas the cleaned up versions assume it only matter what unique transect species were recorded on...]
